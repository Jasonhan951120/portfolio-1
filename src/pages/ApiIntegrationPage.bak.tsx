        ...
        try {
            ...
            if (session?.user?.id) {
                ...
                if (profile?.clinic_id) {
                    ...
                    if (error) {
                        console.error("API Integration: Failed to fetch connections:", error);
                    }
                }
            }
        } catch (err) {
            console.error("API Integration: Error in fetchClinicInfo:", err);
        }
        setIsLoaded(true);
    };
    ...
    return (
        <div className="min-h-screen bg-[#F9FAFB] font-sans relative overflow-x-hidden">
            ...
            <div className="max-w-4xl mx-auto px-6 py-12 relative z-10">
                ...
                {/* ── Loading Overlay ─────────────────────────────────────── */}
                {!isLoaded && (
                    <div className="fixed inset-0 bg-[#F9FAFB]/80 backdrop-blur-sm z-[100] flex items-center justify-center">
                        <div className="flex flex-col items-center gap-4">
                            <div className="w-12 h-12 border-4 border-[#87A96B]/20 border-t-[#87A96B] rounded-full animate-spin" />
                            <p className="text-[11px] font-black uppercase tracking-[0.2em] text-[#87A96B]">Accessing Clinic Data...</p>
                        </div>
                    </div>
                )}
                        ...
                        {PLATFORMS.map((platform, i) => (
                            <motion.div key={platform.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 + i * 0.1 }}>
                                <PlatformCard
                                    platform={platform}
                                    clinicId={clinicId}
                                    initialStatus={connectedPlatforms.includes(platform.id) ? "connected" : "idle"}
                                    onConnectionSuccess={fetchClinicInfo}
                                />
                            </motion.div>
                        ))}
                    </div>
