    ...
    return (
        ...
        >
            ...
            >
                ...
                <div className="relative z-10">
                    <div className="flex justify-between items-start">
                        <div>
                            ...
                            <h3 className="text-[1.8rem] font-bold text-slate-900 tracking-tight leading-none">{lead.name}</h3>
                            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.3em] mt-2">{lead.service}</p>
                        </div>
                        {isVIP && (
                ...
                <div className="relative z-10">
                    ...
                    <div className="flex flex-wrap items-center gap-2 mt-3">
                        {lead.intent_score != null && (
                            <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border transition-all duration-700 ${sent ? 'bg-emerald-50 border-emerald-100 text-emerald-600' : 'bg-slate-50 border-slate-200 text-slate-600'}`}>
                                Intent: {lead.intent_score}%
                            </div>
                        )}
                        {isVIP && (
                            <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-emerald-50 border border-emerald-100 text-emerald-600">
                ...
                {/* ── CONTEXTUAL FOOTER ── */}
                <div className="relative z-10 flex items-center gap-2 mt-auto pt-5 border-t border-slate-100">
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-50 border border-slate-100">
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">🕒 Received {Math.floor(Math.random() * 5) + 1}h ago</span>
                    </div>
                    <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-50 border border-slate-100">
                        <span className="text-[10px] text-slate-500 font-bold uppercase tracking-tight">🦷 {lead.service.split(' ')[0]}</span>
                    </div>
                </div>
                ...
                {lead.ai_summary && (
                    <div className="relative z-10 bg-slate-50 p-5 rounded-[28px] border border-slate-100">
                        <p className="text-xs text-slate-600 leading-relaxed italic">"{lead.ai_summary}"</p>
                    </div>
                )}
                ...
                <div className="relative z-10 space-y-3 mt-auto">
                    <motion.button
                        ...
                        className={`
                            ...
                            transition-all duration-300 flex items-center justify-center gap-3 text-sm
                            ${sent
                                ? 'bg-emerald-500 text-white cursor-default shadow-lg shadow-emerald-200'
                                : 'bg-slate-900 text-white border border-transparent hover:bg-emerald-600 shadow-xl'
                            }
                        `}
                    >
