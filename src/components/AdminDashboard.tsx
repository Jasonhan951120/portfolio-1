...
// Removed static PT_ASSETS, now relying on getTreatmentTemplate dynamically
...
export default function AdminDashboard() {
  ...
  const handleOpenPTMode = useCallback((lead: ConsultationRequest) => {
    ...
    // Lead Architect: Map service to getTreatmentTemplate dynamically
    const asset = getTreatmentTemplate(clinicType, service);
    
    setPtModeData({
      patientName: String(lead?.name || 'Valued Patient'),
      treatmentName: String(asset.title),
      totalPrice: Number(lead?.potential_value || parseInt(asset.investment.replace(/[^0-9]/g, '')) || 5000),
      beforeImage: String(asset.beforeAfter.before),
      afterImage: String(asset.beforeAfter.after)
    });
